...
import 'package:ibivibe/features/events/viewmodels/event_detail_viewmodel.dart';
...
void main() {
  ...
  group('event detail controller', () {
    test('build returns null when no active account', () async {
      ...
      final state = await container.read(eventDetailViewModelProvider('e1').future);
      expect(state, isNull);
    ...
    test('build returns null when event not found', () async {
      ...
      final container = createContainer();
      final state = await container.read(eventDetailViewModelProvider('999').future);
